...
This version replaces the Combo with:
- an Input box for typing the track name (key: -INDEX_SEARCH_INPUT-)
- a Listbox that shows live-filtered indexed filenames (key: -INDEX_MATCHES-)
Type to filter, click to select, then press "Query Indexed".
Double-click/reveal is still disabled for now.
"""
...
layout = [
    ...
    # Search input + listbox to allow typing and live filtering
    ...
    [sg.Table(values=[], headings=["Rank", "Track", "Similarity (%)"], key="-RESULTS-",
              ...
              justification='left', enable_events=False, font=FONT_TABLE)],
...
def save_meta_and_index(filenames_list, hashes_list, embeddings_list, index):
    meta = {
        ...
        "hashes": hashes_list,
        "dim": index.d if index is not None else (embeddings_list[0].shape[0] if embeddings_list else 0),
...
def update_index_search_list():
    """Populate the Listbox with short filenames (no full paths)."""
    short_names = [Path(p).name for p in filenames]
    # Limit number shown if enormous; Listbox will still be scrollable
...
def query_file_gui_external(file_path, top_k):
    ...
    for sim, idx in zip(D[0], I[0]):
        if idx < 0 or idx >= len(filenames):
            continue
        path = filenames[idx]
        results.append([len(results) + 1, Path(path).name, f"{sim * 100:.1f}%"])
    window["-RESULTS-"].update(values=results)

def query_file_gui_indexed(selected_name, top_k):
    """Query using a track already in the index (no re-embedding)."""
    if not selected_name:
        window["-CURRENT_FILE-"].update("No indexed track selected.")
        return
    matched_idx = None
    ...
    try:
        ...
        faiss_index.reconstruct(matched_idx, vec)
        query_vec = vec / (np.linalg.norm(vec) + 1e-9)
    except Exception:
        if matched_idx < len(embeddings_all):
            query_vec = embeddings_all[matched_idx] / (np.linalg.norm(embeddings_all[matched_idx]) + 1e-9)
        else:
            window["-CURRENT_FILE-"].update("Could not reconstruct vector for selected track.")
            return
...
while True:
    ...
    # Live filter as the user types in the search input
        ...
        if q == "":
            ...
            matches = [n for n in short_names if q in n.lower()]
        # limit displayed matches for performance if huge list
        window["-INDEX_MATCHES-"].update(values=matches[:1000])

    # When user clicks a match in the list, populate the input with it
        ...
        # prefer listbox selection; otherwise use the input text to find best match
        sel_list = values.get("-INDEX_MATCHES-")
        sel_name = None
        if sel_list:
            sel_name = sel_list[0]
        else:
            typed = values.get("-INDEX_SEARCH_INPUT-", "").strip()
            if typed:
                # try exact match first, otherwise first substring match
                short_names = [Path(p).name for p in filenames]
