...
while True:
    ...
    if event == sg.WINDOW_CLOSED:
        ...
        if event == "Query This Track":
            # Copy track name into search bar
            track_name = table_data[row_idx][1]
            window["-INDEX_SEARCH_INPUT-"].update(track_name)
            # Trigger the query immediately
            try:
                k = int(values.get("-K-", 5))
            except ValueError:
                k = 5
            query_file_gui_indexed(track_name, k)
